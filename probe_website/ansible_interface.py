...
def get_playbook_status(username, probe=None):
    ...
    will be one of the  following:
        ...
        has-been-updated    : probe has been updated before. will only be returned if
                              updating, completed or failed is not available
        has-not-been-updated: probe has not been updated before.
        unknown             : the log file could not be read
    """

    update_interval = 60
    print(_playbook_status)
    if (_playbook_status is None or
    ...
    if probe is None:
        return _playbook_status['ansible']
    ...
    elif _playbook_status['ansible'] == 'running':
        return 'updating'
